...
public class SavePhoto : MonoBehaviour
{
   ...
   // public Canvas imageGallery_Canvas;
    ...
    public void CaptureScreen()
    {
        ...
        Sprite sprite = SaveImage(SaveCam);
        //instantiante the game object as a child of the imageGallery_canvas
        GameObject photo = Instantiate(photo_GameObject );
    ...
    public string ScreenShotName(int width, int height)
    {
        ...
        savedPhoto = "Assets/Resources/SavedNotes/" + "Note" + i + ".png";
        i++;
        return savedPhoto;
    ...
    public Sprite SaveImage(Camera camera)
    {
        ...
        //take the picture
        Texture2D captureScreen = new Texture2D(resWidth, resHeight, TextureFormat.RGB24, false);
        camera.Render();
        RenderTexture.active = hr;
        captureScreen.ReadPixels(new Rect(0, 0, resWidth, resHeight), 0, 0);

        RenderTexture.active = null; // added to avoid errors
        //encode the screen capture to .png
        byte[] bytes = captureScreen.EncodeToPNG();
